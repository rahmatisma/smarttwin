> ⚠️ **TIDAK SEPENUHNYA RELIABLE — jangan dipercaya buta, verifikasi ke Supabase langsung dulu.**
> Ini rancangan AWAL skema database (draft mentah, belum dirapikan dari transkrip obrolan aslinya — lihat gaya bahasa baris di bawah). Setidaknya SATU bagian sudah terbukti SALAH lewat verifikasi nyata: skema `simulationMetrics` di sini menulis kolom lebar (`delaySeconds`/`queueLengthVeh`/dst + `simulationRunId`), padahal skema ASLI di Supabase (dicek lewat PostgREST OpenAPI) itu pola key-value generik (`simulationId`, `metricName`, `metricValue`, `unit`) — diperbaiki 25 Agustus; lihat `docs/STATUS-DAN-SISA-KERJA.md`. Kode produksi (`SimulationResultWriter.saveMetrics()`) sengaja MENGIKUTI skema live, BUKAN dokumen ini. Kalau butuh skema tabel yang benar, cek langsung ke Supabase (PostgREST OpenAPI / dashboard Supabase), jangan asumsikan dokumen ini akurat untuk tabel lain juga — belum pernah diverifikasi ulang menyeluruh.
>
> Dipertahankan (bukan dihapus) karena masih dirujuk beberapa dokumen historis lain, dan sebagian strukturnya kemungkinan masih relevan sebagai draft awal — tapi anggap "draft belum tervalidasi", bukan "source of truth".
