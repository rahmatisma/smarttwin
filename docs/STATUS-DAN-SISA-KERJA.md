...
| Kandidat "aggressive"/"balanced" cuma tempelan heuristik (+1 detik / rata-rata ke minimum), tidak berlandaskan rumus apa pun — proposal tim menjanjikan evaluasi PKJI 2023 | ✅ **Selesai** (5 September, direvisi 2x hari yang sama) | `scenario_generator.py` — dihitung dari rumus PKJI 2023 (SMP → Flow Ratio → siklus optimum Webster → hijau proporsional untuk "balanced"; + koreksi Degree of Saturation ≤0,85 untuk "aggressive"). `baseline` SENGAJA tidak diubah (dipakai sebagai pembanding before/after di tempat lain). **Arus jenuh (S) per lengan diambil langsung dari studi lapangan Simpang Pingit yang sudah dipublikasikan** (Ramadhani/Sutrisno/Yasin, jurnal Renovasi UST — 5.212/4.490/3.652/3.843 smp/jam Utara/Timur/Selatan/Barat; PDF dibaca lengkap oleh tim, bukan cuma ditranskrip), menggantikan hitungan `600×We` sendiri. Data survei 2016 (~10 tahun), satu-satunya sisa asumsi: proyek tidak mengukur ulang kondisi jalan 2026. Detail + kalimat siap-jawab: `docs/hasil-implementasi-pkji-aggressive-balanced.md`. Diuji SUMO nyata + 8 test PKJI, `pytest backend/tests` 122 passed |
